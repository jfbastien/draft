...
\placeholder{S} is the set of potential exceptions of the entity selected by the
...
\item
Otherwise, \placeholder{S} contains the pseudo-type ``any''.
\end{itemize}

\item
If \tcode{e} implicitly invokes a function (such as an overloaded operator,
an allocation function in a \grammarterm{new-expression}, or a destructor
if \tcode{e} is a full-expression~(\ref{intro.execution})),
\placeholder{S} is the set of potential exceptions of the function.
\item
...
\item
If \tcode{e} is a \grammarterm{new-expression} with a non-constant
\grammarterm{expression} in the \grammarterm{noptr-new-declarator}~(\ref{expr.new}),
\placeholder{S} consists of the type \tcode{std::bad_array_new_length}.
\end{itemize}
...
\item
for \tcode{g()}, the set consists of ``any'';
\item
for \tcode{new A}, the set consists of ``any'';
\item
for \tcode{B()}, the set is empty;
\item
for \tcode{new D}, the set consists of ``any'' and \tcode{double}.
\end{itemize}
\exitexample
...
\pnum
Given an implicitly-declared special member function \tcode{f} of some class \tcode{X},
the \defn{set of potential exceptions of the implicitly-declared special member function} \tcode{f}
...
struct D : public A, public B {
    ...
    // exception specification of \tcode{D::D()} contains \tcode{X} and \tcode{std::bad_array_new_length}

...
\begin{itemize}
\item it is this,
\item it is implicitly captured and
the \grammarterm{capture-default} is \tcode{=}, or
\item it is explicitly captured with a
...
The effect of such an explicit conversion is
...
\exitnote 
The expression \tcode{e} is used as a glvalue if and
...
non-throwing~(\ref{except.spec}), the value of the \grammarterm{new-expression}
...
\grammarterm{new-placement}; such an expression is called a
\defnx{placement \grammarterm{new-expression}}{placement~new-expression@placement \grammarterm{new-expression}}.
...
If both sequences can be formed, or one is formed but it is the
...
\begin{itemize}
  ...
  \item
  if value is of pointer type, it contains
